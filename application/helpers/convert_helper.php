...
/**
 * Fungsi get laboratorium
 */
if (!function_exists('laboratorium')) {
  function laboratorium($id, $flag=0) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('id', $id);
    $row = $ci->db->get('ms_laboratorium')->row_array();
    $name = !empty($row) ? (($flag == 1) ? $row['name'].' ['.$row['kode'].']' : $row['name'])  : '';
    return $name;
  }
}

/**
 * Fungsi get fasilitas
 */
if (!function_exists('fasilitas')) {
  function fasilitas($id) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('id', $id);
    $row = $ci->db->get('ref_fasiltas_pemda')->row_array();
    $name = !empty($row) ? $row['name'] : '';
    return $name;
  }
}
 ...
 * Fungsi get spesimen
 */
if (!function_exists('spesimen')) {
  function spesimen($id) {
    $ci =& get_instance();
    $ci->load->database();
    $ci->db->where('id', $id);
    $row = $ci->db->get('ref_spesimen')->row_array();
    $name = !empty($row) ? $row['name'] : '';
    return $name;
  }
}
