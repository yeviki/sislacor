              ...
              <div class="row">
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Filter Provinsi</b></label>
                    <?php echo form_dropdown('province', $province, $this->input->post('province'), 'class="select-all param" id="province_id"');?>
                  </div>
                </div>
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Filter Kab/Kota</b></label>
                    <?php echo form_dropdown('regency', array(''=>'Pilih Kab/Kota'), $this->input->post('regency', TRUE), 'class="select-all param" id="regency_id"'); ?>
                  </div>
                </div>
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Filter Katogeri Rumah Sakit</b></label>
                    <?php echo form_dropdown('rujukan', array(''=>'Pilih Kategori', 1=>'Rumah Sakit Rujukan', 2=>'Rumah Sakit Daerah'), $this->input->post('rujukan'), 'class="select-all param" id="rujukan"');?>
                  </div>
                </div>
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
                    <label style="font-size:16px;"><b>Filter Status</b></label>
                    <?php echo form_dropdown('status', array(''=>'Pilih Status', 1=>'Aktif', 0=>'Tidak Aktif'), $this->input->post('status'), 'class="select-all param" id="status_id"');?>
                  </div>
                </div>
              </div>
              <div class="row">
...
<script type="text/javascript">
  ...
  function getRegency(provinceId, flag) {
    regeID = (regeID != '') ? regeID : '<?php echo $this->input->post('regency', TRUE); ?>';
    let lblReg = '';
    let name = (flag==1) ? $('#regency') : $('#regency_id');
    $.ajax({
      type: 'GET',
      ...
      success: function(data) {
        ...
        if(data.status == 1) {
          ...
          lblReg = '<option value="">Pilih Kab/Kota</option>';
        name.html(lblReg);
        name.select2('val', regeID).trigger('change');
      }
    });
  }
