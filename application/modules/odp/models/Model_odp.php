...
class Model_odp extends CI_Model
{
	...
	public function getDataKategoriOdp()
	{
		$this->db->order_by('id', 'ASC');
		$query = $this->db->get('ref_kategori_odp');
		return $query->result_array();
	}

	public function cekDataOdpRegencyPerDay()
	{
		$this->db->where('publish_date', $this->_publishDate);
		$this->db->group_by(array('publish_date', 'id_regency'));
		$query = $this->db->count_all_results('ta_odp');
		return $query;
	}

  public function cekDataOdpPublish()
	{
		//set publish_date
		$this->db->where('publish_date', $this->_publishDate);
  ...
  public function count_all($flag)
  {
		...
		$this->db->group_by(array('id_regency', 'publish_date'));
		return $this->db->count_all_results('ta_odp');
  }
  ...
  private function _get_datatables_query($flag)
  {
		...
		$this->db->from('ta_odp a');
		$this->db->join('wa_regency b', 'a.id_regency = b.id', 'inner');
		$this->db->join('ref_kategori_odp c', 'a.id_kategori = c.id', 'inner');
		if($flag == 1)
	...
	public function getDataDetailOdp($id_regency, $publish_date, $flag)
	{
		$status = ($flag == 2) ? array('P') : array('S', 'A');
		$this->db->select('a.id,
											 a.id_regency,
											 ...
											 a.publish_date');
		$this->db->from('ta_odp a');
		$this->db->join('ref_kategori_odp b', 'a.id_kategori = b.id', 'inner');
	...
	public function getDataDetailOdpNew()
	{
		$this->db->select('a.id,
											 a.id_regency,
											 a.id_kategori,
	...
	public function insertDataOdp()
	{
		...
		//cek data odp berdasarkan tgl publish
		$this->db->where('publish_date', $publishDate);
		$this->db->where('id_regency', $id_daerah);
		$qTot = $this->db->count_all_results('ta_odp');
		if($qTot > 0) {
			...
			$this->db->insert_batch('ta_odp', $arrOdp);
			//kirim notifikasi
			...
			$params['reference']		=	'Mengirim rekap data ODP per tanggal '.tgl_indo(date('Y-m-d', strtotime($publishDate)));
	...
	public function updateDataOdp()
	{
		$create_by    = $this->app_loader->current_account();
		$create_date 	= gmdate('Y-m-d H:i:s', time()+60*60*7);
		$create_ip    = $this->input->ip_address();
		$id_daerah    = $this->app_loader->current_regency();
		$id_odp				= $this->encryption->decrypt(escape($this->input->post('odpId', TRUE)));
		$publishDate  = $this->_publishDate;
		//load class notification
		$notif = new notification_manager();
		...
		if($qTot > 0) {
			...
			$this->db->where('id_regency', $id_daerah);
			$this->db->where('id', $id_odp);
			$this->db->update('ta_odp', $data);
			//kirim notifikasi
			$params['sender_id'] 		= $create_by;
			$params['level_akses']	= array(2);
			$params['type']					= 'odp.new';
			$params['parameters']		= $id_daerah;
			$params['reference']		=	'Memperbaharui rekap data ODP per tanggal '.tgl_indo(date('Y-m-d', strtotime($publishDate)));
			$params['create_by']		= $create_by;
			$params['create_date']	= $create_date;
			$params['create_ip']		= $create_ip;
			$notif->add($params);
	...
	public function updateDataOdpAdmin()
	{
		$create_by    = $this->app_loader->current_account();
		$create_date 	= gmdate('Y-m-d H:i:s', time()+60*60*7);
		$create_ip    = $this->input->ip_address();
	...
	public function getDataAkumulasiOdp($today=FALSE)
	{
