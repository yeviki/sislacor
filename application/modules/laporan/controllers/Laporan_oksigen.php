...
class Laporan_oksigen extends SLP_Controller {
	...
	private function history_pemakaian_harian($id_rs, $start_date, $end_date) 
	{
		...
		$dataStok = $this->mLapOksigen->get_stok_tabung($id_rs);
		$log_tersedia = array();
		foreach ($dataStok as $dltk) {
			$log_tersedia[$dltk['id_rs']][$dltk['id_kat_tabung']] = $dltk['total_stok_tabung'];
		}
		...
		foreach($cell as $key => $c) {
			...
			foreach ($kategori as $row => $val) {
				foreach ($val as $rows => $a) {
					$tersedia =isset($log_tersedia[$c['id_rs']][$a['id_kat_tabung']]) ? $log_tersedia[$c['id_rs']][$a['id_kat_tabung']] : '0';
