  ...
  <div class="row" id="formParent">    
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <a type="button" href="<?php echo site_url('vaksinasi/capaian-vaksinasi'); ?>" class="btn btn-inverse" name="button" style="padding:12px 16px;"><b><i class="fa fa-table"></i></b></a>
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="row">
        ...
        <div class="col-xs-12 col-sm-12">
          <?php echo form_open(site_url('vaksinasi/capaian-vaksinasi/upload'), array('id' => 'formupload', 'style'=>'display:none;margin-bottom:20px;')); ?>
            <div style="display:block;background:#FFF;padding:20px;border:1px solid #CCC;box-shadow:0px 0px 10px #CCC;">
              <div class="row">
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
          ...
          <div class="table-responsive">
            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="tblVaksin" width="100%">
              <thead>
                <tr>
                  <th width="3%">#</th>
                  <th width="17%">Tanggal Capaian</th>
                  <th width="17%">Kab/Kota</th>
                  <th width="17%">Dosis</th>
  ...
  <div class="modal-dialog modal-md" id="frmEntry">
    <div class="modal-content">
      <div class="modal-header" style="padding:10px 15px 10px 15px;">
        ...
        <h4 class="modal-title"><b>FORM ENTRI CAPAIAN VAKSINASI</b></h4>
      </div>
      <?php echo form_open(site_url('vaksinasi/capaian-vaksinasi/create'), array('id' => 'formEntry')); ?>
        ...
        <div id="errEntry"></div>
          ...
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="dosis" class="control-label"><b>Dosis <font color="red" size="1em">(*)</font></b></label>
                <?php echo form_dropdown('dosis', $list_dosis, $this->input->post('dosis'), 'class="select-all" id="dosis"');?>
                <div class="help-block"></div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="total_vaksinasi" class="control-label" style="font-size:15px;"><b>Total Vaksinasi <font color="red" size="1em">(*)</font></b></label>
            ...
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="tanggal_vaksinasi" class="control-label"><b>Tanggal <font color="red" size="1em">(*)</font></b></label>
                  <div class="input-group date datemonth">
                    <input type="text" class="form-control mask" name="tanggal_vaksinasi" id="tanggal_vaksinasi" placeholder="dd/mm/yyyy" data-inputmask="'alias': 'date'" value="<?php echo $this->input->post('tanggal_vaksinasi', TRUE); ?>">
                    <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                  </div>
                  <?php echo form_error('tanggal_vaksinasi'); ?>
                <div class="help-block"></div>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
  ...
  function getDataList() {
    $('#tblVaksin').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "vaksinasi/capaian-vaksinasi/listview",
        "type": "POST",
        "data": {
  ...
  function formReset() {
    $('#formEntry').attr('action', site + 'vaksinasi/capaian-vaksinasi/create');
    $('#errEntry').html('');
    $('.select-all').select2('val', '');
  ...
  $(document).on('click', '.btnEdit', function(e){
    formReset();
    $('#formEntry').attr('action', site + 'vaksinasi/capaian-vaksinasi/update');
    var id_Data = $(this).data('id');
    $('#modalEntryForm').modal({
      backdrop: 'static'
    });
    getDataVaksin(id_Data);
  });

  function getDataVaksin(id_Data) {
    run_waitMe($('#frmEntry'));
    $.ajax({
      type: 'POST',
      url: site + 'vaksinasi/capaian-vaksinasi/details',
      data: {'vaksinId' : id_Data, '<?php echo $this->security->get_csrf_token_name(); ?>' : $('input[name="'+csrfName+'"]').val()},
      dataType: 'json',
      success: function(data) {
        $('input[name="'+csrfName+'"]').val(data.csrfHash);
        if(data.status == 1) {
          $('input[name="vaksinId"]').val(id_Data);
          $('#total_vaksinasi').val(data.message.total_vaksinasi);
          $('#tanggal_vaksinasi').val(data.message.tanggal_vaksinasi);
          $('#dosis').select2('val', data.message.id_kat_dosis).trigger('change');
      ...
      buttons: {
        ...
        "main" : {
          ...
          callback:function(response){
            if (response) {
              $.ajax({
                url: site + 'vaksinasi/capaian-vaksinasi/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
