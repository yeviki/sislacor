  ...
  <div class="row" id="formParent">    
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <a type="button" href="<?php echo site_url('vaksinasi/suplai-vaksin'); ?>" class="btn btn-inverse" name="button" style="padding:12px 16px;"><b><i class="fa fa-table"></i></b></a>
    ...
    <div class="col-xs-12 col-sm-12">
      <div class="row">
        ...
        <div class="col-xs-12 col-sm-12">
          <?php echo form_open(site_url('vaksinasi/suplai-vaksin/upload'), array('id' => 'formupload', 'style'=>'display:none;margin-bottom:20px;')); ?>
            <div style="display:block;background:#FFF;padding:20px;border:1px solid #CCC;box-shadow:0px 0px 10px #CCC;">
              <div class="row">
                <div class="col-xs-12 col-sm-3">
                  <div class="form-group">
  ...
  <div class="modal-dialog modal-md" id="frmEntry">
    <div class="modal-content">
      <div class="modal-header" style="padding:10px 15px 10px 15px;">
        ...
        <h4 class="modal-title"><b>FORM ENTRI VAKSIN KELUAR</b></h4>
      </div>
      <?php echo form_open(site_url('vaksinasi/suplai-vaksin/create'), array('id' => 'formEntry')); ?>
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
        <div id="errEntry"></div>
          ...
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="total_suplai" class="control-label" style="font-size:15px;"><b>Total Suplai <font color="red" size="1em">(*)</font></b></label>
                <input type="text" class="form-control nominal" name="total_suplai" id="total_suplai" placeholder="Total" value="<?php echo $this->input->post('total_suplai', TRUE); ?>">
                <div class="help-block"></div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="tanggal_suplai" class="control-label"><b>Tanggal <font color="red" size="1em">(*)</font></b></label>
                  <div class="input-group date datemonth">
                    <input type="text" class="form-control mask" name="tanggal_suplai" id="tanggal_suplai" placeholder="dd/mm/yyyy" data-inputmask="'alias': 'date'" value="<?php echo $this->input->post('tanggal_suplai', TRUE); ?>">
                    <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                  </div>
...
<script type="text/javascript">
  ...
  function getDataList() {
    $('#tblVaksin').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "vaksinasi/suplai-vaksin/listview",
        "type": "POST",
        "data": {
  ...
  function formReset() {
    $('#formEntry').attr('action', site + 'vaksinasi/suplai-vaksin/create');
    $('#errEntry').html('');
    $('.select-all').select2('val', '');
  ...
  $(document).on('click', '.btnEdit', function(e){
    formReset();
    $('#formEntry').attr('action', site + 'vaksinasi/suplai-vaksin/update');
  ...
  function getDataVaksinMasuk(id_vaksin_masuk) {
    ...
    $.ajax({
      type: 'POST',
      url: site + 'vaksinasi/suplai-vaksin/details',
      ...
      success: function(data) {
        ...
        if(data.status == 1) {
          ...
          $('#total_suplai').val(data.message.total_suplai);
          $('#tanggal_suplai').val(data.message.tanggal_suplai);
      ...
      buttons: {
        ...
        "main" : {
          ...
          callback:function(response){
            if (response) {
              $.ajax({
                url: site + 'vaksinasi/suplai-vaksin/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
  ...
  $(document).on('click', '.download', function(e){
        url = site + '/repository/template/import_data_suplai_vaksin.xlsx';
  ...
  $(document).on('submit', '#formupload', function(e) {
        ...
        $.ajax({
            url: site + 'vaksinasi/suplai-vaksin/upload',
            type: 'post',
            data: form_data,
