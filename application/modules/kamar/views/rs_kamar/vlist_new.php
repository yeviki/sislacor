  ...
  <div class="row" id="formParent">    
    <div class="col-xs-12 col-sm-12">
      <div class="btn-toolbar" style="margin-bottom: 15px">
        <a type="button" href="<?php echo site_url('kamar/rs-kamar'); ?>" class="btn btn-inverse" name="button" style="padding:12px 16px;"><b><i class="fa fa-table"></i></b></a>
                ...
                <div class="col-xs-12 col-sm-3">
                    <div class="form-group">
                        ...
                        <div class="input-group date datemonth">
                          <input type="text" class="form-control mask" name="tanggal_kamar" id="tanggal_kamar" placeholder="dd/mm/yyyy" data-inputmask="'alias': 'date'" value="<?php echo $this->input->post('tanggal_kamar', TRUE); ?>">
  ...
  <div class="modal-dialog modal-lg" id="frmEntry">
    <div class="modal-content">
      <div class="modal-header" style="padding:10px 15px 10px 15px;">
        ...
        <h4 class="modal-title"><b>FORM ENTRI KAMAR</b></h4>
      </div>
      <?php echo form_open(site_url('kamar/rs-kamar/create'), array('id' => 'formEntry')); ?>
      <div class="modal-body" style="padding:15px 15px 5px 15px;">
        <div id="errEntry"></div>
          <div class="row">
          <?php
              echo form_input(array('type'=>'hidden', 'name'=>'rsId', 'id'=>'idRegency'));
              echo form_input(array('type'=>'hidden', 'name'=>'publishDate', 'id'=>'datetanggal'));
            ?>
            <div class="col-xs-12 col-sm-6">
              <div class="form-group required">
                <label for="id_rs_input" class="control-label"><b>Rumah Sakit <font color="red" size="1em">(*)</font></b></label>
                <?php echo form_dropdown('id_rs_input', $list_id_rs, $this->input->post('id_rs_input'), 'class="select-all" id="id_rs_input"');?>
                <div class="help-block"></div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-6">
          ...
          <div class="table-responsive">
            <table cellspacing="0" cellpadding="0" class="table table-striped table-bordered" width="100%" id="tblOtgInput">
              ...
              <?php $i=1; foreach ($list_kamar as $key => $k): ?>
                <tr>
                  <td><?php echo $i; ?>.</td>
                  <td id="<?php echo 'otg_desc_'.$i; ?>"><?php echo $k['nm_kamar']; ?></td>
                  <td>
                    <input type="text" class="form-control nominal" name="param[<?php echo $k['id_kat_kamar']; ?>]" id="<?php echo 'otg_jum_'.$i; ?>" placeholder="Jumlah Kamar" value="<?php echo set_value('param['.$k['id_kat_kamar'].']', ''); ?>" required="" oninvalid="this.setCustomValidity('Inputan wajib diisi')" oninput="setCustomValidity('')">
                  </td>
                </tr>
              <?php $i++; endforeach; ?>
              </tbody>
...
<script type="text/javascript">
  ...
  function getDataList() {
    $('#tblVaksin').dataTable({
      ...
      "ordering": false,
      "ajax": {
        "url": site + "kamar/rs-kamar/listview",
        "type": "POST",
        "data": {
  ...
  $(document).on('click', '.btnEdit', function(e){
    ...
    $('#id_rs_input').select2('val', rsId).trigger('change');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.status != 0) {
          let no = 1;
          $.each(data.message, function(key,value){
            $('#tblOtgInput > tbody > tr').find('td#otg_desc_'+no).text(value['nm_kamar']);
            $('#tblOtgInput > tbody > tr').find('td > #otg_jum_'+no).val(value['total_kamar']);
            no++;
          });
        }
        $('#frmEntry').waitMe('hide');
  ...
  //     backdrop: 'static'
  //   });
  //   getDataVaksinMasuk(id_rs, tanggal);
  // });

  // function getDataVaksinMasuk(id_rs, tanggal) {
  //   run_waitMe($('#frmEntry'));
  //   $.ajax({
  //     type: 'POST',
  //     url: site + 'kamar/rs-kamar/details',
  //     data: {'rsId' : id_rs, 'tanggalId' : tanggal,'<?php echo $this->security->get_csrf_token_name(); ?>' : $('input[name="'+csrfName+'"]').val()},
  //     dataType: 'json',
  //     success: function(data) {
  //       $('input[name="'+csrfName+'"]').val(data.csrfHash);
  //       if(data.status == 1) {
  //         $('input[name="rsId"]').val(id_rs);
  //         $('input[name="tanggalId"]').val(tanggal);
      ...
      buttons: {
        ...
        "main" : {
          ...
          callback:function(response){
            if (response) {
              $.ajax({
                url: site + 'kamar/rs-kamar/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
