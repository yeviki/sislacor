...
class Otg extends SLP_Controller {

	public function __construct()
  {
    parent::__construct();
		$this->load->model(array('model_otg' => 'motg', 'master/model_master' => 'mmas'));
    ...
    $this->breadcrumb->add('Dashboard', site_url('home'));
		$this->breadcrumb->add('Rekap OTG', '#');

		$this->session_info['page_name'] = "Rekap OTG";
		$this->session_info['jadwal'] 	 = $jadwal;
		$this->session_info['deadline']  = $waktu;
		$this->session_info['data_otg']  = $this->motg->cekDataOtgPublish();
		$this->session_info['regency']   = $this->mmas->getDataRegency();
		$this->session_info['kategori']  = $this->motg->getDataKategoriOtg();
		if($this->app_loader->is_kesreg())
    	$this->template->build('vlist_opr', $this->session_info);
		else {
			$this->session_info['tot_otg'] = $this->motg->cekDataOtgRegencyPerDay();
   		...
   		exit('No direct script access allowed');
			...
			if(!empty($session) AND !empty($param)) {
				...
				if($data['message'] == 'ERROR') {
					...
					$tot_otg = $this->motg->cekDataOtgRegencyPerDay();
					$result = array('status' => 1, 'message' => array('reg' => '<b>'.$data['note'].'</b>', 'isi' => ' berhasil disimpan...'), 'csrfHash' => $csrfHash);
					$this->cipusher->send(array('message'=>'otgsuccess', 'status'=>'new', 'tot_otg'=>$tot_otg));
				}
			} else {
   		...
   		exit('No direct script access allowed');
			...
			if(!$this->app_loader->is_kesreg())
				...
				if($flag == 2)
					...
					$data = $this->motg->getDataDetailOtgNew();
					if(count($data) > 0) {
						$row = array(); $no=1;
						foreach ($data as $key => $val) {
							...
							$row['no'] 				= $no;
							$row['kategori']	= $val['desc'].' per tanggal '.$jadwal;
							...
							$row['action']    = ($val['status'] == 'S') ? '<button class="btn btn-xs btn-orange btnUpdate" data-id="'.$this->encryption->encrypt($val['id']).'" title="Edit Data OTG"><i class="fa fa-pencil"></i></button>' : '<button class="btn btn-xs btn-orange disabled" title="Edit Data OTG"><i class="fa fa-pencil"></i></button>';
							$hasil[] = $row;
							$no++;
						}
	...
	private function listViewAll()
	{
		...
		if(isset($session)){
			$flag  = escape($this->input->post('flag',TRUE));
			$data_otg = $this->motg->get_datatables($flag);
			$no = $this->input->post('start');
			foreach ($data_otg as $do) {
				$no++;
				$row = array();
				$row[] = $no;
				$row[] = $do['name'];
				...
				$row[] = '<button class="btn btn-sm btn-green btnDetail" data-id="'.$this->encryption->encrypt($do['id_regency']).'" data-date="'.$do['publish_date'].'" data-flag="'.$flag.'" data-reg="'.$do['name'].'" title="Rincian Rekapitulasi Data OTG"><i class="fa fa-eye"></i></button>';
				$data[] = $row;
			}

			$output = array(
				"draw" => $this->input->post('draw'),
				"recordsTotal" => $this->motg->count_all($flag),
				"recordsFiltered" => $this->motg->count_filtered($flag),
				"data" => $data,
			);
		}
		...
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			if(!empty($id_regency) AND !empty($publish_date) AND !empty($session)) {
				...
				$data = $this->motg->getDataDetailOtg($this->encryption->decrypt($id_regency), $publish_date, $flag);
				if(count($data) > 0) {
					$row = array(); $no=1;
					foreach ($data as $key => $val) {
						$row['no'] 				= $no;
	...
	private function updateKesreg()
	{
		if (!$this->input->is_ajax_request()) {
			...
			$session  = $this->app_loader->current_account();
			$csrfHash = $this->security->get_csrf_hash();
			$otgId    = escape($this->input->post('otgId', TRUE));
			if(!empty($otgId) AND !empty($session)) {
				$data = $this->motg->updateDataOtg();
				if($data['message'] == 'ERROR') {
					...
					$result = array('status' => 1, 'message' => ' berhasil diperbaharui...', 'csrfHash' => $csrfHash);
					$this->cipusher->send(array('message'=>'otgsuccess', 'status'=>'update'));
				}
			} else {
		...
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			if(!empty($session) AND !empty($regency) AND !empty($param)) {
				$data = $this->motg->updateDataOtgAdmin();
				if($data['message'] == 'ERROR') {
	...
	public function akumulasi()
	{
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			if(!empty($session)) {
				$arrNew = array();
				$dataNew = $this->motg->getDataAkumulasiOtg(TRUE);
				foreach ($dataNew as $key => $dn) {
					$arrNew[$dn['key_column']] = $dn['jumlah'];
				}
				$new = !empty($arrNew[1]) ? $arrNew[1] : 0;
				$end = !empty($arrNew[2]) ? $arrNew[2] : 0;
				$sts = (!empty($arrNew[3]) ? $arrNew[3] : 0) + (!empty($arrNew[4]) ? $arrNew[4] : 0) + (!empty($arrNew[5]) ? $arrNew[5] : 0);
				$arrLast = array();
				$dataLast = $this->motg->getDataAkumulasiOtg();
				...
				$totSts = (!empty($arrLast[3]) ? $arrLast[3] : 0) + (!empty($arrLast[4]) ? $arrLast[4] : 0) + (!empty($arrLast[5]) ? $arrLast[5] : 0);
