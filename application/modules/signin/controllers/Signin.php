...
class Signin extends MY_Controller {
  ...
  private function is_captcha($total_error) {
	...
	private function set_session($username, $group)
	{
		...
		if(count($dataUser) > 0) {
			$session['nama_user']  			= $dataUser['nama_user'];
			...
			$session['userid']			    = $this->encryption->encrypt($dataUser['id_users']);
			$session['regional']				= $dataUser['id_regency'];
			$session['hospital']				= $dataUser['id_rs'];
			$session['laboratorium']		= $dataUser['id_labor'];
			//simpan session
  ...
  private function set_group_session($username)
	...
	{
		$username 	= $this->session->userdata('account_name');
		$session_id = $this->session->userdata('TanggapCovid19@911solop_session');
