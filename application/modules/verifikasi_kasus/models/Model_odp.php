...
class Model_odp extends CI_Model
{
  ...
  private function _get_datatables_query($flag)
  {
	...
	public function approveDataOdp()
	{
		...
		$dataOdp = $this->db->get('ta_odp')->result_array();
		if(count($dataOdp) <= 0)
			...
			$params['recipient_id']	= !empty($dataOdp) ? $dataOdp[0]['create_by'] : $notif->getUserByRegency($id_daerah);
			$params['type']					= 'odp.verified';
			$params['parameters']		= $id_daerah;
			$params['reference']		=	'Rekap data ODP per tanggal '.tgl_indo(date('Y-m-d', strtotime($publishDate))).' telah kami verifikasi';
			...
			$paramNew['reference']		=	regency($id_daerah).' telah melaporkan data terbaru rekap ODP per tanggal '.tgl_indo(date('Y-m-d', strtotime($publishDate)));
