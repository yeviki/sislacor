...
class Kasus_otg extends SLP_Controller {
	...
	public function __construct()
  {
    ...
    $this->breadcrumb->add('Dashboard', site_url('home'));
		$this->breadcrumb->add('Verifikasi Kasus', '#');
		$this->breadcrumb->add('Kasus OTG', '#');

		$this->session_info['page_name'] = "Kasus OTG";
		...
		$this->template->build('form_otg/vlist', $this->session_info);
	}
		...
		if (!$this->input->is_ajax_request()) {
   		exit('No direct script access allowed');
			...
			if(!empty($session) AND !empty($regency)) {
				$data = $this->motg->approveDataOtg();
				if($data['message'] == 'SUCCESS') {
					$result = array('status' => 1, 'message' => 'Proses verifikasi data rekapitulasi OTG <b>'.$data['note'].'</b> berhasil...', 'csrfHash' => $csrfHash);
					$this->cipusher->send(array('message'=>'otgverified'));
				} else {
					$result = array('status' => 0, 'message' => 'Proses verifikasi data rekapitulasi OTG <b>'.regency($this->encryption->decrypt($regency)).'</b> gagal, karena data sudah diverifikasi sebelumnya...', 'csrfHash' => $csrfHash);
				}
			} else {
